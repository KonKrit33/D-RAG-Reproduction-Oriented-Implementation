...
def validate_epoch(
                    ...
                    try:
                        ...
                        if args.val_log_samples > 0 and len(logged_samples) < args.val_log_samples:
                            logged_samples.append({
                                ...
                                "retrieved": bool(any_relevant_retrieved),
    ...
    with open("predictions.json", "w", encoding="utf-8") as f:
        json.dump(logged_samples, f, ensure_ascii=False, indent=2)  
    return out


def evaluate_phase2(args):
    ...
    if args.phase1_checkpoint and os.path.exists(args.phase1_checkpoint):
        ...
        if "retriever_state_dict" in checkpoint:
            state_dict = checkpoint["retriever_state_dict"]   # Phase 2 checkpoint
        else:
            state_dict = checkpoint["model_state_dict"]       # Phase 1 checkpoint
        ...
        loss = checkpoint.get("loss", checkpoint.get("train_loss", "N/A"))
        print(f"  Checkpoint from epoch {checkpoint.get('epoch', '?')}, loss: {loss}")
    else:
    ...
    model_id=args.generator_checkpoint or args.generator_model,
    revision=args.generator_revision,
    use_lora=True,
    )
    projector = Projector(
    gnn_dim=fact_embed_dim,
    nemotron_dim=generator.model.config.hidden_size,
    ).to(device)
    phase2_ckpt = torch.load(
    "checkpoints_webqsp_phase2_clean_qwen/phase2_best.pt",
    map_location=device,
    )
    projector.load_state_dict(phase2_ckpt["projector_state_dict"])
    print("Loaded Phase 2 projector.") 
    ...
    print("\nSkipping gradient flow verification due to CPU/disk offloading.")
    gradient_ok = True    
    # 7. Setup Optimizer
    ...
    if val_dataloader is None:
        raise ValueError("No test/validation dataloader provided. Use --val_heuristics_path.")
...
if __name__ == "__main__":
    ...
    type=str,
    default=None,
    help="Path to a fine-tuned generator checkpoint (generator_best)"
    )
    args = parser.parse_args()
    
    evaluate_phase2(args)
